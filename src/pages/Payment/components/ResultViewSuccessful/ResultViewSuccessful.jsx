...
import { updateWalletService } from '../../../../services/apis/payment.service';
import { useSelector } from 'react-redux';
import { useEffect } from 'react';

const ResultSuccessfully = ({ leadingToHomepage, leadingToUserProfile, money }) => {
  const user = useSelector(state => state.auth.user);
  const updateWallet = async ({ money }) => {
    try {
      await updateWalletService({ money: money, tutorId: user?.id });
